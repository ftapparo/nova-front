...
export default function Exaustores() {
  ...
  const onTurnOn = async () => {
    ...
    if (!moduleId) {
      notify.warning("Apartamento inválido", { description: "Informe um apartamento com final de 1 a 8." });
      return;
    }

    const cachedModule = exhaustDevices.find((device) => device.id.trim().toUpperCase() === moduleId);
    if (!cachedModule) {
      notify.warning("Módulo não encontrado no cache", {
        description: `Não foi possível localizar o módulo ${moduleId}. Atualize os equipamentos e tente novamente.`,
      });
      return;
    }

    if (!cachedModule.online) {
      notify.error("Módulo offline", {
        description: `${cachedModule.nome || moduleId} está offline. Comando de ligar não foi enviado.`,
      });
      return;
    }

    notify.success("Módulo online", { description: `${cachedModule.nome || moduleId} pronto para acionamento.` });
  ...
  return (
    <PageContainer>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <Button
          variant="outline"
          onClick={() => void processStatusQuery.refetch()}
          disabled={processLoading}
          className="h-9 max-sm:h-8 max-sm:w-8 max-sm:border-0 max-sm:bg-transparent max-sm:p-0"
          aria-label="Atualizar"
        >
          <RefreshCw className={`h-4 w-4 ${processLoading ? "animate-spin" : ""} sm:mr-2`} />
            ...
            <div className="mt-4 flex w-full justify-end">
              <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
                ...
                <Button
                  ...
                  className="h-11 w-full px-7 text-[0.95rem] font-semibold sm:min-w-[152px] sm:w-auto"
                >
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="typo-section-title">Exaustores Ligados</h2>
        </div>
        ...
        {sortedActiveExhausts.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4 sm:[grid-template-columns:repeat(auto-fit,minmax(260px,292px))]">
            {sortedActiveExhausts.map((exhaust) => (
              <Card key={exhaust.id} className="rounded-2xl border-border shadow-sm">
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between gap-3">
                    <CardTitle className="typo-card-title-strong">
                      {exhaust.tower}-{exhaust.final}
                    </CardTitle>

                    <div className="flex items-center gap-1">
                      <Badge variant="outline" className="mt-0.5 rounded-full border-none state-success-soft px-3 py-1.5 typo-caption font-medium">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem
                            onClick={() => setExhaustToConfirmOff(exhaust)}
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 divide-x divide-border/80 typo-body">
                    ...
                    <div className="space-y-2 pl-4">
                      ...
                      <p className="typo-caption">
                        Relé: <span className="font-semibold text-foreground">{exhaust.relay}</span>
                      </p>
                    </div>
                  </div>
